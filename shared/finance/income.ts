...
// โมดูลกลาง: ใช้ร่วมกันทั้ง frontend และ backend เพื่อให้ตัวเลข/การจัดหมวดตรงกันเสมอ
...
export const INCOME_40_LABELS: string[] = INCOME_40.map(c => c.label)
export const INCOME_FREQS: IncomeFreq[] = ['รายเดือน', 'รายปี']

/** label เดิม (ก่อนใช้หมวด 40) ที่หมายถึงเงินก้อนรายปี */
export const LEGACY_ANNUAL_LABEL = 'โบนัส'
...
export function migrateIncomeLabel(label?: string): string {
  ...
  if (INCOME_40_LABELS.indexOf(l) >= 0) return l
...
type IncomeRow = { freq?: string | null; label?: string | null; amount?: any }

/** รายการนี้เป็น "รายปี" หรือไม่ (freq ใหม่ · เผื่อ backward-compat กับ label 'โบนัส' เดิม) */
export function isAnnualIncome(row: IncomeRow): boolean {
  ...
  return String(row.label || '').trim() === LEGACY_ANNUAL_LABEL
}

/** จำนวนต่อเดือน (สำหรับสรุปกระแสเงินสด) */
export function monthlyIncome(row: IncomeRow): number {
  const amt = toNum(row.amount)
  return isAnnualIncome(row) ? amt / 12 : amt
}

/** จำนวนทั้งปี */
export function annualIncome(row: IncomeRow): number {
  const amt = toNum(row.amount)
  return isAnnualIncome(row) ? amt : amt * 12
}

/** รวมรายได้ต่อเดือนจากทุกแหล่ง (ข้ามแถวที่ยอด ≤ 0) */
export function sumMonthlyIncome(rows: IncomeRow[] | null | undefined): number {
  return (rows ?? []).reduce((s, r) => (toNum(r?.amount) > 0 ? s + monthlyIncome(r) : s), 0)
}

/** แถว "เงินเดือน" ที่ระบบดึงมาอัตโนมัติ (auto) · เผื่อ label 'เงินเดือน' เดิม */
export function findSalaryRow<T extends IncomeRow & { auto?: boolean }>(rows: T[] | null | undefined): T | undefined {
  return (rows ?? []).find(r => r?.auto || r?.label === 'เงินเดือน')
}
