...
describe('profileSchema — สมมติฐานที่ผิดจะทำให้ทั้งแผนเพี้ยน', () => {
  ...
  it('ยอมรับช่องตัวเลขที่ว่าง ("") — ลูกค้าไม่มีคู่สมรส ช่องอายุคู่สมรสว่าง ต้องบันทึกได้ (regression)', () => {
    expect(ok(S.profileSchema, { retirementAgeSpouse: '', lifeExpectancySpouse: '' })).toBe(true)
    expect(ok(S.profileSchema, { inflationRate: '', expectedReturn: '' })).toBe(true)
    // ค่าที่ระบุจริงยังตรวจช่วงตามเดิม
    expect(bad(S.profileSchema, { retirementAgeSpouse: 200 })).toBe(true)
  })
})
