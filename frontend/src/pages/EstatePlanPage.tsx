...
export default function EstatePlanPage({ person = 'self' }: { person?: Person }) {
  ...
  const { allocs, ruleNote, noHeir } = useMemo(() => {
    ...
    if (nChild > 0) {
      ...
      children.forEach((c, i) => out.push({ name: c.name || `บุตรคนที่ ${i + 1}`, role: c.age != null && c.age < 20 ? `บุตร (ผู้เยาว์ อายุ ${c.age})` : 'บุตร (ผู้สืบสันดาน)', heirShare: each, color: '#06b6d4' }))
