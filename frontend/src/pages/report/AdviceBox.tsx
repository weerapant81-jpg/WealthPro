...
import { X, Check, Pencil, GripHorizontal } from 'lucide-react'
...
function AdviceDialog({ title, value, onSave, onClose }: {
  ...
  // ── ลากย้าย dialog ด้วยแถบหัว ──
  // กล่องพิมพ์บังเนื้อหาที่กำลังอ่านอยู่ จึงต้องเลื่อนหลบได้ · ระหว่างลากแก้ style ของ DOM ตรง ๆ
  // ไม่ setState เพื่อไม่ให้ textarea ถูกวาดใหม่ทุก pointermove (ซึ่งจะทำให้เคอร์เซอร์/โฟกัสสะดุด)
  const drag = useRef<{ px: number; py: number; ox: number; oy: number } | null>(null)
  const at = useRef({ x: 0, y: 0 })

  const onPointerDown = (e: React.PointerEvent) => {
    if (e.button !== 0) return
    drag.current = { px: e.clientX, py: e.clientY, ox: at.current.x, oy: at.current.y }
    e.currentTarget.setPointerCapture(e.pointerId)
  }
  const onPointerMove = (e: React.PointerEvent) => {
    const d = drag.current
    if (!d || !panelRef.current) return
    at.current = { x: d.ox + (e.clientX - d.px), y: d.oy + (e.clientY - d.py) }
    panelRef.current.style.transform = `translate(${at.current.x}px, ${at.current.y}px)`
  }
  ...
  return (
    <div className="no-print" onClick={onClose}
      ...
      <div ref={panelRef} onClick={e => e.stopPropagation()}
        ...
        {/* แถบหัว = ที่จับสำหรับลากย้ายกล่อง */}
        <div onPointerDown={onPointerDown} onPointerMove={onPointerMove} onPointerUp={endDrag} onPointerCancel={endDrag}
          title="ลากเพื่อย้ายกล่อง"
          style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, cursor: 'grab', touchAction: 'none', userSelect: 'none' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <button onPointerDown={e => e.stopPropagation()} onClick={onClose}
              style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex' }}><X size={18} /></button>
          </div>
        </div>
...
/**
 * กล่องคำแนะนำบนหน้ากระดาษ — คลิกแล้วเปิด dialog ให้พิมพ์ (ตัว dialog ลากย้ายได้ที่แถบหัว)
